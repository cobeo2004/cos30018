if __name__ == "__main__":
    ...
    # XGBoostInstance is a singleton instance of XGBoostModel
    xgb_train = XGBoostInstance.train(
        x_train.reshape(x_train.shape[0], x_train.shape[1] * len(FEATURE_COLS)), y_train
    )
    xgb_predict = XGBoostInstance.predict(
        x_test.reshape(x_test.shape[0], x_test.shape[1] * len(FEATURE_COLS))
    )
    # Check NaN
    ...
    # RandomForestInstance is a singleton instance of RandomForestModel
    rf_train = RandomForestInstance.train(
        x_train.reshape(x_train.shape[0], x_train.shape[1] * len(FEATURE_COLS)), y_train
    )
    rf_predict = RandomForestInstance.predict(
        x_test.reshape(x_test.shape[0], x_test.shape[1] * len(FEATURE_COLS))
    )
    ...
    xgb_predict_inverse = scalers["TargetNextClose"].inverse_transform(
        xgb_predict.reshape(-1, 1)
    )
    rf_predict_inverse = scalers["TargetNextClose"].inverse_transform(
        rf_predict.reshape(-1, 1)
    )

    y_test_inverse = scalers["TargetNextClose"].inverse_transform(y_test.reshape(-1, 1))

    PredictionResultInstance.plot(
        y_test_inverse, xgb_predict_inverse, rf_predict_inverse
    )

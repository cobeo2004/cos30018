...
from src.utils import FredInstance


def load_data(
    start: Annotated[str, Doc("The start date of the stock to be loaded")],
    end: Annotated[str, Doc("The end date of the stock to be loaded")],
    ticker: Annotated[
        str, Doc("The ticker symbol of the stock to be loaded")
    ] = "CBA.AX",
):
    """
    ...
    try:
        ...
        if not check_directory_exists(RAW_DATA_DIRECTORY):
            ...
            macroeconomic_data = FredInstance.get_macroeconomic_data(start, end)
            data = data.join(macroeconomic_data, how="left")
            ...
            if not check_file_exists(RAW_CSV_FILE):
                ...
                data = pd.read_csv(RAW_CSV_FILE, index_col=0, parse_dates=True)
        return data
